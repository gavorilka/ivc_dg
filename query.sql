...
--                    LEFT JOIN VALS V304 ON V304.OBJ_ID = L36.OBJ_ID AND V304.PARAM_ID = 304 AND V304.IS_DEL = 0 -- Полное название ОУ КПК
            ...
            --            '", "years": "' || 
            --            CASE 
            --                WHEN E.TOTAL_MONTHS < 0 THEN E.TOTAL_YEARS - 1 
            --                ELSE E.TOTAL_YEARS 
            --            END ||
            --            '", "months": "' || 
            --            CASE 
            --                WHEN E.TOTAL_MONTHS < 0 THEN MOD(E.TOTAL_MONTHS + 12, 12) 
            --                ELSE MOD(E.TOTAL_MONTHS, 12) 
            --            END || 
            --            '", "days": "' || 
            --            CASE 
            --                WHEN E.TOTAL_DAYS < 0 THEN MOD(E.TOTAL_DAYS + 30, 30) 
            --                ELSE MOD(E.TOTAL_DAYS, 30) 
            --            END || '"}' 
            --            , ', ') || ']' AS EXPERIENCE
            --    FROM (
            --        SELECT 
            --            L85.PARENT_ID,
            --            V465.LISTVAL_ID AS LISTVAL_ID,
            --            SUM(CASE WHEN DATEDIFF(YEAR FROM V466.val TO COALESCE(V467.val, CURRENT_DATE)) > 0 
            --                     THEN DATEDIFF(YEAR FROM V466.val TO COALESCE(V467.val, CURRENT_DATE)) 
            --                     ELSE 0 END) AS TOTAL_YEARS,
            --            SUM(CASE WHEN DATEDIFF(MONTH FROM V466.val TO COALESCE(V467.val, CURRENT_DATE)) > 0 
            --                     THEN DATEDIFF(MONTH FROM V466.val TO COALESCE(V467.val, CURRENT_DATE)) 
            --                          - (DATEDIFF(YEAR FROM V466.val TO COALESCE(V467.val, CURRENT_DATE)) * 12) 
            --                     ELSE 0 END) AS TOTAL_MONTHS,
            --            SUM(CASE WHEN DATEDIFF(DAY FROM 
            --                DATEADD(MONTH, DATEDIFF(MONTH FROM V466.val TO COALESCE(V467.val, CURRENT_DATE)), V466.val) 
            --                TO COALESCE(V467.val, CURRENT_DATE)) > 0 
            --                     THEN DATEDIFF(DAY FROM 
            --                DATEADD(MONTH, DATEDIFF(MONTH FROM V466.val TO COALESCE(V467.val, CURRENT_DATE)), V466.val) 
            --                TO COALESCE(V467.val, CURRENT_DATE)) 
